...
# Deploy Timesheet MCP Server v2 to Azure Web App
# Azure Web App Name: mcp-timesheet-mcp-v2-1234
#
...
echo "Deploying Timesheet MCP Server v2 to Azure Web App..."
echo "Using SUFFIX: $SUFFIX"

# Set variables using the same pattern as existing scripts
RESOURCE_GROUP="mcp-python-demo-rg-${SUFFIX}"
WEB_APP_NAME="mcp-timesheet-mcp-v2-${SUFFIX}"
ASP_NAME="mcp-demo-asp-${SUFFIX}"
LOCATION="${REGION:-eastus}"
SOURCE_DIR="timesheet_app/mcp_server_v2"
...
if curl -sf "https://$WEB_APP_NAME.azurewebsites.net/health" >/dev/null 2>&1; then
  echo "✓ Timesheet MCP Server v2 is responding (/health)"
elif curl -sf "https://$WEB_APP_NAME.azurewebsites.net/" >/dev/null 2>&1; then
  echo "✓ Timesheet MCP Server v2 is responding (/)"
else
  echo "✗ Timesheet MCP Server v2 is not responding yet (may need more time)"
fi
